...
export default function EventRegistrationCard({
  ...
  return (
    <motion.div
      layout
      className={`relative group h-full flex flex-col rounded-4xl transition-all duration-700 overflow-hidden ${
        isSelected
    ...
    >
      ...
      <div
        className={`absolute inset-0 transition-all duration-1000 ${
          isSelected
            ? "opacity-[0.12]"
            : "opacity-[0.05] group-hover:opacity-[0.08]"
      ...
      <div
        className={`relative flex-1 flex flex-col border rounded-4xl transition-all duration-500 backdrop-blur-2xl ${
          isSelected
            ? "bg-black/60 border-[#BA170D]/50"
            : "bg-white/3 border-white/10 group-hover:border-white/20"
        } ${isRegistrationClosed && !isSelected ? "opacity-60 grayscale-[0.5]" : ""}`}
      >
                ...
                <button
                  onClick={() => {
                    if (isRegistrationClosed) {
                      if (isGroup && isSelected) setIsExpanded(!isExpanded);
                      return;
                    }
                    isGroup ? setIsExpanded(!isExpanded) : onToggle();
                  }}
                  disabled={(isDisabled || isRegistrationClosed) && !isSelected}
                  className={`group/btn relative flex items-center gap-3 px-8 py-3.5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all duration-500 overflow-hidden shadow-2xl ${
                    isSelected
                ...
                {isSelected ? (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between px-1">
                      <span className="text-[11px] font-black text-[#BA170D] uppercase tracking-[0.3em]">
                        Team Roster
                      </span>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {teamDetails?.members.map((m, i) => (
                        <div
                          key={i}
                          className={`flex items-center justify-between p-4 rounded-3xl border transition-all ${m.role === "leader" ? "bg-[#BA170D]/10 border-[#BA170D]/40" : "bg-white/5 border-white/10"}`}
                        >
                          <div className="flex items-center gap-4 min-w-0">
                            <div
                              className={`w-11 h-11 rounded-2xl flex items-center justify-center text-sm font-black shrink-0 ${m.role === "leader" ? "bg-[#BA170D] text-white" : "bg-white/10 text-gray-200"}`}
                            >
                              {m.role === "leader" ? "L" : "M"}
                            </div>
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] text-gray-400 font-mono truncate tracking-tight">
                                {m.email}
                              </p>
                            </div>
                          </div>
                    ...
                    {!isRegistrationClosed && (
                      <button
                        onClick={onLeaveTeam}
                        className="w-full py-4 text-[10px] font-black text-gray-400 hover:text-red-500 uppercase tracking-[0.3em] transition-all border border-white/10 rounded-2xl hover:bg-red-500/10"
                      >
                        Leave/Disband Team
                      </button>
                    )}
                  ...
                  <div className="space-y-8">
                    <div className="space-y-3">
                      <span className="text-[11px] font-black text-white uppercase tracking-[0.3em] pl-1">
                        Add Teammates
                      </span>
                      ...
                      {searchError && (
                        <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest pl-2 pt-1 flex items-center gap-1.5">
                    ...
                    {teamMembers.length > 0 && (
                      <div className="space-y-3 pr-1">
                        {teamMembers.map((m, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-3xl"
                          >
                            <div className="flex items-center gap-4 min-w-0">
                              <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center text-[10px] font-black text-gray-100">
                                {idx + 2}
                              </div>
                              <span className="text-xs font-black text-white truncate max-w-[150px] uppercase">
                                {m.name || m.email}
                              </span>
                            </div>
                            <button
                              ...
                              className="p-2.5 text-gray-300 hover:text-red-500 transition-all rounded-full hover:bg-red-500/10"
                            >
                              <X size={20} />
                            </button>
                          </div>
                    ...
                    <button
                      ...
                      disabled={
                        teamMembers.length === 0 || (isDisabled && !isSelected)
                      }
            ...
            >
              <div className="p-6 md:p-8 border-b border-white/10 flex justify-between items-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <h3 className="text-xl md:text-2xl font-black font-unbounded text-white uppercase tracking-tighter leading-tight">
                    {event.title}
                  </h3>
                  <p className="text-[#BA170D] text-[10px] font-black uppercase tracking-[0.4em] mt-2">
                    The Guidelines
                  </p>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 gap-5">
                  {event.rules.map((rule, idx) => (
                    <div key={idx} className="flex gap-4 group/rule">
                      <div className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-[#BA170D] font-black text-sm shrink-0">
                        0{idx + 1}
                      </div>
                      <p className="text-[11px] text-gray-200 font-bold leading-relaxed pt-2.5 uppercase tracking-tight group-hover:text-white transition-colors">
                        {rule}
                      </p>
                    </div>
                  ))}
              ...
              <div className="p-6 bg-white/1 border-t border-white/10">
                <button
                  onClick={() => setShowRules(false)}
                  className="w-full py-4 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-[0.4em] hover:bg-[#BA170D] hover:text-white transition-all"
                >
                  Understand
                </button>
              </div>
